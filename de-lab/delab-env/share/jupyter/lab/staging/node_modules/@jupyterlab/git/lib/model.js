...
export class GitExtension {
    ...
    constructor(docmanager = null, docRegistry = null, settings) {
        ...
        this._branchesChanged = new Signal(this);
        ...
        this._dirtyFilesStatusChanged = new Signal(this);
        this._credentialsRequiredChanged = new Signal(this);
    ...
    async getRemotes() {
        ...
        const result = await this._taskHandler.execute('git:show:remote', async () => {
            return await requestAPI(URLExt.join(path, 'remote', 'show'), 'GET');
        });
    ...
    async removeRemote(name) {
        ...
        await this._taskHandler.execute('git:remove:remote', async () => {
            await requestAPI(URLExt.join(path, 'remote', name), 'DELETE');
        });
    }
    /**
    ...
    async diff(previous, current) {
        ...
        const data = await this._taskHandler.execute('git:diff', async () => {
            return await requestAPI(URLExt.join(path, 'diff'), 'POST', {
                previous,
                current
            });
        });
    ...
    async fetch(auth) {
        ...
        const data = this._taskHandler.execute('git:fetch:remote', async () => {
            return await requestAPI(URLExt.join(path, 'remote', 'fetch'), 'POST', {
                auth: auth
            });
        });
        return data;
    ...
    async refreshBranch() {
        ...
        try {
            ...
            const branchesChanged = !JSONExt.deepEqual(this._branches, ((_a = data.branches) !== null && _a !== void 0 ? _a : []));
            this._branches = (_b = data.branches) !== null && _b !== void 0 ? _b : [];
    ...
    async refreshDirtyStatus() {
        ...
        for (const fileName of fileNames) {
            const docWidget = this._docmanager.findWidget(this.getRelativeFilePath(fileName));
            if (docWidget !== undefined) {
    ...
    get markedFiles() {
        return this._currentMarker.markedFilePaths
            ...
            .map(path => this.status.files.find(fileStatus => fileStatus.to === path));
    }
    /**
