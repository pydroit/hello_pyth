...
async function activate(app, mainMenu, restorer, factory, renderMime, settingRegistry, docmanager, statusBar, translator) {
    ...
    try {
        ...
        showErrorMessage(trans.__('Failed to load the jupyterlab-git server extension'), error.message, [Dialog.warnButton({ label: trans.__('Dismiss') })]);
        return null;
    }
    ...
    if (settings) {
        ...
        const gitPlugin = new GitWidget(gitExtension, settings, app.commands, fileBrowser.model, trans);
        ...
        addFileBrowserContextMenu(gitExtension, fileBrowser, app.contextMenu, trans);
    }
    // Register diff providers
    gitExtension.registerDiffProvider('Nbdime', ['.ipynb'], (model, toolbar, translator) => createNotebookDiff(model, renderMime, toolbar, translator));
    return gitExtension;
}
