import { Dialog, ReactWidget, showDialog, showErrorMessage, Toolbar, ToolbarButton } from '@jupyterlab/apputils';
...
export function addCommands(app, gitModel, fileBrowserModel, settings, translator) {
    ...
    commands.addCommand(ContextCommandIDs.openFileFromDiff, {
        ...
        execute: async (_) => {
            ...
            const matches = toArray(shell.widgets('main')).filter(widget => widget.id === domNode.dataset.id);
            if (matches.length === 0) {
                return;
            }
 ...
 */
export function addFileBrowserContextMenu(model, filebrowser, contextMenu, trans) {
    let gitMenu;
    let _commands;
    let _paths;
